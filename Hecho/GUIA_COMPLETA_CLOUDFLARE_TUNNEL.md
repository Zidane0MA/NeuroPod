...
### 8.3. Problemas con WebSockets (Jupyter Lab)

Si tienes problemas con aplicaciones que usan WebSockets (como Jupyter Lab):

1. En Cloudflare, ve a "Configuración" > "Network".
2. Asegúrate de que WebSockets esté activado.
3. En la configuración de `config.yml`, añade:
   ```yaml
   ingress:
     - hostname: "jupyter-*.neuropod.online"
       service: http://localhost:443
       originRequest:
         noTLSVerify: true
         connectTimeout: 10s
         disableChunkedEncoding: true  # Puede ayudar con ciertos problemas
   ```
